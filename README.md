...
> **AI エージェントはまず `edinet schema commands` を実行してください。** 全コマンドのフラグ・型・使用例が JSON で返ります。
...
```bash
edinet schema commands             # 全コマンド・フラグ・型・使用例（JSON）
edinet schema doc-types            # 書類種別コード一覧（42種、例: 120=有価証券報告書）
edinet schema sections             # テキスト抽出可能なセクション一覧
edinet schema financial-elements   # 財務XBRL要素マッピング一覧
edinet schema derived-metrics      # 派生財務指標（ROE, ROA等）の計算式一覧
```
...
# 2. 直近の有価証券報告書を探す
edinet company filings 7203 --doc-type 120 --limit 3

# 3. 財務データを構造化抽出（複数期間）
edinet company financials 7203 --periods 5 --summary-only

# 4. 特定書類のリスク情報をテキスト抽出
edinet doc text S100XXXX --section risk
```
...
# 今日の開示書類を一覧表示
edinet doc list --date 2026-03-31

# 有価証券報告書のみに絞り込み
edinet doc list --date 2026-03-31 --doc-type 120
...
# 企業の財務サマリーを複数期間取得
edinet company financials 7203 --periods 3 --summary-only
```
...
# 提出者名と書類説明でフィルタ
edinet doc list --date 2025-06-20 --filer-name トヨタ --doc-description 有価証券報告書
```
...
# サマリー指標のみ（詳細行なし）
edinet doc financial S100ABCD --summary-only
...
edinet company search --industry 自動車
```
...
```bash
edinet company filings 7203 --doc-type 120 --limit 5
edinet company filings E00010 --from 2025-01-01 --to 2025-03-31
```
...
```bash
# 直近3期分の全財務諸表
edinet company financials 7203

# 直近5期分の損益計算書サマリー
edinet company financials 7203 --periods 5 --statement pl --summary-only

# 企業名でも指定可能
edinet company financials トヨタ --periods 3
```
...
全42種のコード一覧は `edinet schema doc-types` で取得できます。
...
全指標の詳細は `edinet schema derived-metrics` で取得できます。
